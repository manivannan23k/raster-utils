...
import { assign, endsWith, forEach, invert, times, isTypedFloatArray, isTypedUintArray, isTypedIntArray } from './utils.js';
...
const encodeImage = (values, width, height, metadata, _metadata) => {
...
//   const img = new Uint8Array(values.buffer);
  ...
  // const data = new Uint8Array(numBytesInIfd + (4 * width * height * samplesPerPixel));
  const data = new Uint8Array(numBytesInIfd + (values.length * _metadata.BitsPerSample[0]/8 * samplesPerPixel));
  times(prfx.length, (i) => {
    data[i] = prfx[i];
  });
  // forEach(img, (value, i) => {
  ...
  for (let i = 0, vl = values.length; i < vl; i++) {
    ...
    const buffer = new ArrayBuffer(_metadata.BitsPerSample[0]/8)
    const view = new DataView(buffer)
    
    if(_metadata.SampleFormat[0]===1){
        if(_metadata.BitsPerSample[0]===8){
            view.setUint8(0, value, false)
        }
        if(_metadata.BitsPerSample[0]===16){
            view.setUint16(0, value, false)
        }
        if(_metadata.BitsPerSample[0]===32){
            view.setUint32(0, value, false)
        }
    }
    if(_metadata.SampleFormat[0]===2){
        if(_metadata.BitsPerSample[0]===8){
            view.setInt8(0, value, false)
        }
        if(_metadata.BitsPerSample[0]===16){
    ...
    if(_metadata.SampleFormat[0]===3){
        if(_metadata.BitsPerSample[0]===32){
            view.setFloat32(0, value, false)
        }
        if(_metadata.BitsPerSample[0]===64){
            view.setFloat64(0, value, false)
        }
    }

    const uint8 = new Uint8Array(view.buffer)
    const idx = numBytesInIfd + i * _metadata.BitsPerSample[0]/8;

    for(let k=0;k<_metadata.BitsPerSample[0]/8;k++){
        let _idx = idx + k;
        data[_idx] = uint8[k]
    }
  }
